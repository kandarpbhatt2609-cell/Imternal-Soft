...
const CategoryPage = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get(`/auth/api/user/categories/${categoryName}`)
      .then(res => {
    ...
    <SiteLayout>
      ...
      {loginPrompt && (
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={() => navigate('/user/login?redirect=' + encodeURIComponent(window.location.pathname))}
      ...
      <div style={{ padding:'36px 0 20px' }}>
        ...
        <div style={{ display:'flex', alignItems:'baseline', gap:14 }}>
          <h1 style={{ margin:0, fontSize:'2rem', fontWeight:800, color:'#253d4e', textTransform:'capitalize' }}>{categoryLabel}</h1>
          {!loading && (
        ...
        <p style={{ margin:'6px 0 0', color:'#94a3b8', fontSize:14 }}>Click any product to view details and add to cart.</p>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:'center', padding:'80px 0', color:'#94a3b8' }}>
          ...
          <h3 style={{ fontWeight:700, color:'#475569', margin:'0 0 8px' }}>No Products Found</h3>
          <p style={{ margin:0, fontSize:14 }}>No products available in this category yet.</p>
        </div>
      ) : (
